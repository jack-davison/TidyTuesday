...
ggthemr::ggthemr("flat", text_size = 14, layout = "scientific", spacing = 1)
...
tuesdata$student_debt %>%
  ...
  gghighlight::gghighlight(use_direct_label = F, unhighlighted_params = list(fill = "white", color = "white")) +
  ggthemr::scale_colour_ggthemr_d() +
  theme(legend.position = "none",
        text = element_text(family = "Bahnschrift"),
        plot.title = element_textbox_simple(lineheight = 1, padding = margin(0, 0, 5, 0)),
        plot.title.position = "plot",
        plot.caption = element_text(size = 8, color = "grey66", hjust = .5),
        plot.caption.position = "plot") +
  scale_y_continuous(sec.axis = sec_axis(~., labels = scales::dollar),
